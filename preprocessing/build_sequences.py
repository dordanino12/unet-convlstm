    ...
    for i in tqdm(chunk_indices, desc="Time Chunks"):
        ...
        for s_idx in range(NUM_SAMPLES):
            ...
            for folder_name in batch_folders:
                ...
                try:
                    # 1. Load Images
                    with open(f_v0, 'rb') as f: d0 = pickle.load(f)
                    with open(f_v1, 'rb') as f: d1 = pickle.load(f)
                    ...
                    # 2. Load Map
                    with open(f_map, 'rb') as f: dm = pickle.load(f)
                    
                    # A. Fix NaNs
                    w_raw = np.nan_to_num(dm['w_map'], nan=0.0)
                    
                    # B. RESIZE (256 -> 128)
                    # ------------------------------------------------------
                    w_raw = cv2.resize(w_raw, (128, 128), interpolation=cv2.INTER_AREA)
                    # ------------------------------------------------------
                    
                    w_map = w_raw[np.newaxis, ...].astype(np.float32)

                    seq_inputs.append(img_stack)
                    seq_targets.append(w_map)
