...
OUTPUT_VIDEO = os.path.join(parent_dir, "plots", "Paper54_Supplementary_Video.mp4")

MODELS_CFG = {
    "Envelope": {
        "npz": os.path.join(parent_dir, "data", "wacv_data/envelop_kfold_w_sensor_noise_both/test_w.npz"),
        "ckpt": os.path.join(parent_dir, "models", "wacv/envelop/mit_b1_envelop_fold_01_val_r0-1_c0-2_best_bin_loss.pt"),
    },
    "500m": {
...
def add_title_frames(video_writer, frame_size, fps, seconds):
    ...
    for _ in range(n_frames):
        ...
        ax.text(0.5, 0.58, "Inferring Vertical Air Flow by Spaceborne Imaging",
                ha='center', va='center', fontsize=32, fontweight='bold', color='#1e1e1e')
        
        ax.text(0.5, 0.38, "Paper ID #54",
                ha='center', va='center', fontsize=26, fontweight='bold', color='#5a5a5a')
...
def make_part_geometry_and_render(video_writer, seq_for_render, csv_times, sat_lookup, fixed_limits, frame_size):
    ...
    for t in range(t_max):
        ...
        time_sec = t * 20
        fig.suptitle(f"Geometry and Images | Time: {time_sec} sec", fontsize=20, fontweight="bold")
        ...
        ax_geo.set_title("3D Satellite Geometry", fontsize=18, fontweight="bold")
        ax_geo.axis("off")

        ax_sat0.imshow(sat_a, cmap="gray", vmin=0, vmax=1)
        ax_sat0.set_title("Image at Sat A", fontsize=16, fontweight="bold")
        ax_sat0.axis("off")

        ax_sat1.imshow(sat_b, cmap="gray", vmin=0, vmax=1)
        ax_sat1.set_title("Image at Sat B", fontsize=16, fontweight="bold")
        ax_sat1.axis("off")
...
def make_part_gt_pred_grid(video_writer, prepared_data, frame_size):
    labels = ["Envelope", "500m", "1000m", "1500m"]
    ...
    # This ensures that any masked/NaN pixels are rendered strictly as black!
    ...
    for t in range(t_max):
        ...
        time_sec = t * 20
        fig.suptitle(f"GT vs Inferred Vertical Velocity | Time: {time_sec} sec", fontsize=20, fontweight="bold")

        for col, label in enumerate(labels):
            data = prepared_data[label]
            gt = data["gt"][t, 0]
            pred = data["pred"][t, 0]
            mask_seq = data.get("mask", None)
            vmin = data["vmin"]
            vmax = data["vmax"]
            norm = per_label_norm[label]

            # Apply mask only if it exists (for the Envelope mode)
            if mask_seq is not None:
                # Handle dimension shape just in case (T, 1, H, W) vs (T, H, W)
                mask_t = mask_seq[t, 0] if mask_seq.ndim == 4 else mask_seq[t]
                
                # Mask out areas where mask == 0 (np.ma.masked_where treats True as "hide this pixel")
                gt = np.ma.masked_where(mask_t == 0, gt)
                pred = np.ma.masked_where(mask_t == 0, pred)

            im_gt = axes[0, col].imshow(gt, cmap=cmap_custom, norm=norm, interpolation='nearest')
            axes[0, col].set_title(f"{label} GT [m/s]", fontsize=14, fontweight="bold")
            axes[0, col].axis("off")
            ...
            axes[1, col].set_title(f"{label} Inferred [m/s]", fontsize=14, fontweight="bold")
            axes[1, col].axis("off")
...
def main():
    ...
    print("[INFO] Loading datasets and models (Envelope/500m/1000m/1500m)...")
    prepared = {}
    seq_for_render = None
    ...
    for label, cfg in MODELS_CFG.items():
        ...
        # Extract mask_seq here!
        input_seq, gt_seq_norm, mask_seq = dataset[SEQUENCE_IDX]
        ...
        # Store the mask ONLY for the Envelope mode (so other slices show everything)
        if label == "Envelope":
    ...
    warmup_fig = plt.figure(figsize=(18, 8), dpi=120)
