...
MAX_PLOT_POINTS = 50000  # Downsample for the scatter plot
...
if len(scatter_gt_list) > 0:
    ...
    # --- 1. SCATTER PLOT ---
    if len(all_gt) > MAX_PLOT_POINTS:
        indices = np.random.choice(len(all_gt), MAX_PLOT_POINTS, replace=False)
        x_scatter = all_gt[indices]
        y_scatter = all_pred[indices]
    else:
        x_scatter = all_gt
        y_scatter = all_pred

    c_error = np.abs(y_scatter - x_scatter)
    sort_idx = np.argsort(c_error)
    
    sc = ax_scatter.scatter(x_scatter[sort_idx], y_scatter[sort_idx], c=c_error[sort_idx], 
                            s=4, cmap='plasma', vmin=0, vmax=2.0, alpha=0.6)
    
    ax_scatter.plot([-7.5, 7.5], [-7.5, 7.5], 'k--', lw=1.5)
    ax_scatter.set_xlabel("Ground Truth [m/s]")
    ax_scatter.set_ylabel("Predicted [m/s]")
    ax_scatter.set_title(f"Scatter Plot (MAE: {global_mae:.3f})")
    ax_scatter.set_xlim(-7.5, 7.5)
    ax_scatter.set_ylim(-7.5, 7.5)
    ax_scatter.grid(True, alpha=0.3)
    plt.colorbar(sc, ax=ax_scatter, label="Abs Error")
    ...
    for t in sorted(unique_times):
        mask_t = (all_time == t)
        if np.any(mask_t):
            time_mae.append(np.mean(np.abs(all_diff[mask_t]))) 
            time_steps.append(t)
            
    ax_time.plot(time_steps, time_mae, 'o-', color='darkblue', lw=2)
    ax_time.set_xlabel("Time Step")
    ax_time.set_ylabel("MAE [m/s]")
    ax_time.set_title("Mean Absolute Error over Sequence Time")
    ...
    # --- NEW: UNIFY Y-AXIS FOR GT AND PRED ---
    # Get current limits
    _, y_max_gt = ax_hist_gt.get_ylim()
    _, y_max_pred = ax_hist_pred.get_ylim()
    
    # Find global max density to keep scales identical
    common_y_max = max(y_max_gt, y_max_pred)
    
    # Set both to the same limit
    ...
    save_path = "evaluation_comprehensive.png"
