...
# - Model: shared MiT-B1 encoder + ConvLSTM bottleneck (pretrained) and one decoder+head per height (each outputs 3 channels)
# - y_pred returned by the model has shape: [B, T, Nh, 3, H, W]
# - Loss: bin-weighted L1 (same idea as main.py) applied to flattened channels, plus small gradient term
# - Saves best checkpoint and prints per-height + aggregated metrics
# """
...
# #         self.Y = data['Y'].astype(np.float32)  # (N,T,Nh,3,H,W)
# #         if self.X.ndim != 5:
# #             raise ValueError(f"Unexpected X dims: {self.X.shape}")
# #         if self.Y.ndim != 6:
# #             raise ValueError(f"Unexpected Y dims: {self.Y.shape}")
# #
# #         self.N, self.T, self.C, self.H, self.W = self.X.shape
# #         # Y: (N,T,Nh,3,H,W)
# #         self.Ny, self.Ty, self.Nh, self.C3, self.Hy, self.Wy = self.Y.shape
...
# #         # For each (height,channel) compute max_abs = max(|min|, |max|) and use scale = max_abs / target_norm.
# #         # This keeps physical 0.0 exactly 0.0.
...
# #                     # Degenerate -> keep scale 1.0 (division will be safe and values are zero)
...
# #         print(f"[DATA] Y uses per-height/per-channel symmetric max-absolute normalization to map +/-{'{:.3f}'.format(np.max(self.y_maxabs.flat))} -> +/-{self.target_norm:.3f}")
# #
# #     def __len__(self):
# #         return self.N
# #
# #     def __getitem__(self, idx):
# #         x = torch.from_numpy(self.X[idx])  # (T,C,H,W)
# #         y_raw = self.Y[idx]  # (T,Nh,3,H,W) numpy
# #
# #         # Normalize X
# #         x = x / self.norm_const
# #
...
# #                 # symmetric max-abs normalization: divide by per-slice scale; zero remains zero
...
# #         if arr.ndim == 5:
# #             # Support flattened layout: [B, T, Nh*3, H, W]
...
# #             for c in range(self.C3):
# #                 scale = self.y_scale[h, c]
# #                 # invert symmetric normalization: multiply by scale (no offset)
...
#         # We'll keep it and add an additional Conv2d layer to expand channels from 3 to Nh
...
#         else:
#             x_enc = x_flat
#         # Encoder returns list of features
#         features = self.encoder(x_enc)
#
#         # Bottleneck and temporal LSTM (replicates PretrainedTemporalUNetMitB1 behavior)
...
#     Simple bin-weighted L1 loss without mask logic.
#     Returns scalar loss tensor.
#     """
#     # Flatten channels: new C = Nh * 3
#     B, T, Nh, C3, H, W = y_pred.shape
#     C = Nh * C3
#     y_p = y_pred.reshape(B, T, C, H, W)
#     y = y_true.reshape(B, T, C, H, W)
#
#     # Denormalize for bin computations
...
#
#     # Flatten pixel values for bin counting
...
#
#     # Apply spatial mask weighting if requested (simple approach)
...
#
#     # Per-channel losses (u/v/w): compute with same weighting but sliced per channel
#     per_channel_losses = []
#     for c in range(C3):
#         # Slice [B,T,Nh,H,W]
#         y_p_c = y_pred[:, :, :, c, :, :]
...
#
#     all_mae = []
#     per_channel_mae = [[] for _ in range(dataset_obj.C3)]
...
#
#         # Metrics: denormalize and compute MAE per height and aggregated
#         with torch.no_grad():
#             y_den = dataset_obj.denormalize(y).cpu().numpy()
#             pred_den = dataset_obj.denormalize(y_pred).cpu().numpy()
#             # shapes: [B,T,Nh,3,H,W]
#             diff = np.abs(pred_den - y_den)
#             all_mae.extend(diff.flatten())
#             # per-channel mean absolute error (u/v/w)
#             for c in range(dataset_obj.C3):
#                 per_channel_mae[c].extend(diff[:, :, :, c, :, :].flatten())
#
#     avg_loss = total_loss / n if n > 0 else 0.0
#     agg_mae = float(np.mean(all_mae)) if len(all_mae) > 0 else 0.0
#     per_c_mae = [float(np.mean(pc)) if len(pc) > 0 else 0.0 for pc in per_channel_mae]
...
#     p.add_argument('--train', default='/home/danino/PycharmProjects/pythonProject/data/3d_688to702_train_uvw.npz' ,help='Training NPZ path')
#     p.add_argument('--val', default='/home/danino/PycharmProjects/pythonProject/data/3d_688to702_val_uvw.npz' , help='Validation NPZ path')
#     p.add_argument('--test', default='/home/danino/PycharmProjects/pythonProject/data/3d_688to702_test_uvw.npz' , help='Test NPZ path')
...
#
#     # Use each dataset's own per-height/per-channel Y stats; only X normalization is shared from train.
#     val_ds.norm_const = train_ds.norm_const
#     test_ds.norm_const = train_ds.norm_const
#
#     # Quick sanity check: normalize -> denormalize should recover the original Y values.
#     sample_y = np.array(train_ds.Y_ref[:1])
...
#
#     # derive bin bounds from train data (raw values, not denormalized)
#     y_all = np.asarray(train_ds.Y_ref, dtype=np.float32)
#     # Y is already in physical units, so use directly
#     bin_min = float(np.min(y_all))
#     bin_max = float(np.max(y_all))
#     print(f"Derived BIN bounds: {bin_min:.4f} to {bin_max:.4f}")
#
...
#
#     # ConvLSTM: include main lstm and any skip lstms
#     convlstm_params = 0
...
#         # lstm_skips can be a ModuleList
#         try:
#             for m in model.lstm_skips:
#                 convlstm_params += _count_params(m)
#         except Exception:
#             # fallback if it's a single module
#             convlstm_params += _count_params(model.lstm_skips)
#
#     # Decoders/heads (now per-velocity type)
#     decoder_params = 0
#     head_params = 0
#     if hasattr(model, 'decoders') and model.decoders is not None:
#         decoder_params = sum(_count_params(m) for m in model.decoders)
#     if hasattr(model, 'heads') and model.heads is not None:
#         head_params = sum(_count_params(m) for m in model.heads)
#
#     total_params = _count_params(model)
#     trainable_params = _count_trainable_params(model)
#
#     # Print a human-friendly breakdown with short explanations
...
#             best_val = val_loss
#             # Overwrite the same filename to avoid saving many checkpoints
...
#
#     # Final test with best model (only load if we saved at least once)
#     if best_saved:
